...
# 구글 SERP에서 www.coupang.com 링크가 잘 붙는 브랜드·로켓 계열 위주(불안정 문구 제외).
...
def _mode2_google_nav_query_sequence(explicit: Optional[str]) -> List[str]:
    """
    None/빈 문자열: 풀에서 무작위 1개 후, 실패 시 '쿠팡' 폴백.
    비어 있지 않으면: 해당 문자열 1회 후(쿠팡이 아니면) '쿠팡' 폴백.
    """
    s = (explicit or "").strip()
    if s:
...
def _google_search_one_query_open_coupang(cc: CoupangCrawler, page: Page, google_query: str) -> Page:
    """단일 구글 검색어로 SERP에서 쿠팡 링크 클릭까지. 실패 시 예외."""
    ...
    if do_refresh_serp:
        try:
            safe_print("[MODE2] Google SERP refresh 1회 수행")
            page.reload(wait_until="domcontentloaded", timeout=25000)
            page.wait_for_timeout(900)
        except Exception as re_serp:
            safe_print(f"[MODE2] SERP refresh 실패(무시): {re_serp!r}")
...
def _google_search_and_open_coupang(
    cc: CoupangCrawler,
    page: Page,
    google_query: Optional[str] = None,
    *,
    nav_log: Optional[Callable[[str], None]] = None,
) -> Page:
    """구글 검색 후 SERP에서 쿠팡 링크 클릭. 매 진입마다 안정 풀에서 무작위 1차 쿼리, 실패 시 '쿠팡' 재시도."""
    ...
    seq = _mode2_google_nav_query_sequence(google_query)
    _emit(f"[MODE2] google nav sequence={seq!r}")

    last_err: Optional[Exception] = None
    for attempt_i, q in enumerate(seq):
        if attempt_i > 0:
            _emit(f"[MODE2] google nav retry query={q!r}")
        try:
            return _google_search_one_query_open_coupang(cc, page, q)
        except Exception as e:
            last_err = e
            if attempt_i >= len(seq) - 1:
                break
            try:
                page.goto("https://www.google.com/", wait_until="domcontentloaded", timeout=25000)
                page.wait_for_timeout(400)
                cc._accept_google_consent_if_present(page)
            except Exception as re_g:
                _emit(f"[MODE2][WARN] google 재진입 실패: {re_g!r}")
    raise RuntimeError(f"쿠팡 링크 진입 실패(시도 {seq!r}): {last_err!r}") from last_err
...
def run_mode2_sequential_blocking(
    ...
    *,
    ...
    google_query: None이면 안정 풀에서 무작위 1차 검색어, 실패 시 '쿠팡' 폴백.
    반환: 키워드별 요약 dict 리스트.
    """
    ...
    if strict_clean:
        _log("[MODE2] strict clean mode ON: profile/cache/session reset")
        for pdir in [getattr(cc, "_chrome_user_data_dir", ""), getattr(cc, "_prep_user_data_dir", "")]:
            try:
                ...
                _log(f"[MODE2][WARN] profile dir reset failed: {pdir} {ce!r}")
        try:
            state_path = (_ROOT / ".smoke" / "coupang_state.json").resolve()
            if state_path.is_file():
                state_path.unlink(missing_ok=True)
        except Exception as se:
            _log(f"[MODE2][WARN] storage_state delete failed: {se!r}")
    ...
    try:
        ...
        if strict_clean:
            try:
                ...
                _log(f"[MODE2][WARN] clear_cookies failed: {ce!r}")
        ...
        cc._accept_google_consent_if_present(page)
        page = _google_search_and_open_coupang(cc, page, google_query, nav_log=_log)

        page.wait_for_timeout(400)
        _coupang_mouse_circle_if_demo(page)

        for idx, kw in enumerate(kws):
            if idx > 0 and stop_requested and stop_requested():
                _log("[MODE2] stop requested: current keyword completed, halting before next keyword")
                break
            if idx > 0:
...
def run_mode2_chunked_sessions_blocking(
    cc: CoupangCrawler,
    keywords: List[str],
    *,
    batch_token: str,
    keywords_per_session: Optional[int] = None,
    inter_session_sleep_min_sec: float = 10.0,
    inter_session_sleep_max_sec: float = 20.0,
    google_query: Optional[str] = None,
    retry_blocked: bool = False,
    log: Optional[Callable[[str], None]] = None,
    stop_requested: Optional[Callable[[], bool]] = None,
    ...
    """
    키워드를 `keywords_per_session`개씩 끊어, 창(세션)을 매 청크마다 새로 열고 연속 수집한다.
    청크 사이에는 `inter_session_sleep_*` 범위에서 랜덤 휴식(기본 10~20초)을 둔다.

    - `keywords_per_session`이 None, 0 이하, 또는 전체 개수 이상이면 **한 세션**으로 처리(기존과 동일).
    - 입력 키워드는 순서 유지하며 앞에서부터 중복 텍스트를 한 번씩만 사용한다.
    - `google_query`가 None이면 세션(창)마다 안정 풀에서 무작위 구글 검색어 후, 실패 시 '쿠팡' 폴백.
    """
    _ensure_windows_proactor_policy()
    ...
    raw = [str(k or "").strip() for k in keywords if str(k or "").strip()]
    seen: set[str] = set()
    kws: List[str] = []
    for k in raw:
        if k not in seen:
    ...
    n = len(kws)
    kps = int(keywords_per_session) if keywords_per_session is not None else 0
    if kps <= 0 or kps >= n:
        chunk_size = n
    else:
        chunk_size = max(1, min(kps, n))

    chunks: List[List[str]] = [kws[i : i + chunk_size] for i in range(0, n, chunk_size)]
    total_sessions = len(chunks)

    if total_sessions <= 1:
        _log(f"[MODE2] chunked runner: single session ({n} keyword(s)), chunk_size={chunk_size}")
    ...
    lo = float(inter_session_sleep_min_sec)
    hi = float(inter_session_sleep_max_sec)
    if hi < lo:
        lo, hi = hi, lo

    _log(
        f"[MODE2] chunked runner: {n} keyword(s) → {total_sessions} session(s), "
        f"up to {chunk_size} per session, inter-session rest {lo:.1f}~{hi:.1f}s"
    )

    all_results: List[Dict[str, Any]] = []
    global_ord = 0
    for si, chunk in enumerate(chunks):
        if si > 0 and stop_requested and stop_requested():
            _log("[MODE2] stop requested: finished previous session, aborting remaining sessions")
            break
        if si > 0:
            rest = random.uniform(lo, hi)
            _log(
                f"[MODE2] inter-session rest {rest:.2f}s before session {si + 1}/{total_sessions} "
                f"({len(chunk)} keyword(s))"
            )
            time.sleep(rest)

        part = run_mode2_sequential_blocking(
            cc,
            chunk,
            batch_token=batch_token,
            google_query=google_query,
        ...
        for row in part:
            if not isinstance(row, dict):
                continue
            global_ord += 1
            row["mode2_session"] = si + 1
            row["mode2_sessions_total"] = total_sessions
            row["mode2_global_idx"] = global_ord
